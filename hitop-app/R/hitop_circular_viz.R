...
plot_spectrum_detail <- function(bars_t, spectrum_name,
  ...
  else max(score_range[2],
           ceiling(10 * suppressWarnings(
             max(c(bt$mean, bt$hi), na.rm = TRUE))) / 10)
  span    <- ceil_y - floor_y
  ...
  p +
    ...
    { if (ceil_y > score_range[2] && !tscore)
        geom_vline(xintercept = score_range[2], color = "grey60",
                   linewidth = 0.35, linetype = "22") } +
...
plot_hitop_horizontal <- function(bars, defs = NULL,
  ...
  ceil_y  <- if (tscore)
    max(t_ceil, 5 * ceiling(max(c(bars$mean, bars$hi), na.rm = TRUE) / 5))
           ...
           ceiling(10 * suppressWarnings(
             max(c(bars$mean, bars$hi), na.rm = TRUE))) / 10)
  span    <- ceil_y - floor_y
  breaks  <- if (tscore) seq(40, ceil_y - 5, 10) else seq(score_range[1], score_range[2], 1)
  ...
  p +
    ...
    { if ("est" %in% names(d))
        geom_point(data = d |> filter(!is.na(est)),
                   aes(x = pmin(pmax(est, floor_y), ceil_y), y = ypos),
                   shape = 23, size = 1.4, fill = "white",
                   color = "grey15", stroke = 0.45) } +
    geom_point(data = d |> filter(capped),
